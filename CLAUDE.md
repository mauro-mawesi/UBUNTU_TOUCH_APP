...
17. **Qt 5.12 V4 silently ignores the `timeZone` option of `Date.toLocaleString` / `Intl.DateTimeFormat`** (limited ECMAScript Intl support). The call returns the system timezone instead of erroring. This made `get_current_time({timezone:"Asia/Tokyo"})` return the user's local time. Use the `TimeUtil` C++ helper (`src/timeutil.{h,cpp}`, registered as `Ragassistant.Util 1.0`) which delegates to `QTimeZone` for full IANA support. The helper is injected into the JS tool registry via `Tools.setContext({ timeUtil })` because `.pragma library` modules can't reach QML-registered types directly.
